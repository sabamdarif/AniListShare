(function () {
  ...
  function getCSRF() {
    // prefer hidden input, fallback to cookie (Django's standard AJAX pattern)
  ...
  let _debounce = null;
  let _selectedName = ""; // name to save to DB (from suggestion pick)
  let _rating = 0;
  let _seasons = []; // [{ total, watched, comment }]
        ...
        <div class="aam_body">
          ...
          <div class="aam_name_wrap">
            <div class="aam_name_field">
              <i class="nf nf-seti-search aam_name_icon"></i>
              <input class="aam_name_input" type="text" placeholder="Anime Name" autocomplete="off">
              <span class="aam_search_spinner"></span>
            </div>
            <div class="aam_suggestions"></div>
          </div>

          <!-- thumbnail + suggestions row -->
          ...
          <div class="aam_section">
            <div class="aam_lang_wrap">
              ...
              <div class="aam_lang_input_wrap">
                <input class="aam_lang_input" type="text" placeholder="Type to add language…" autocomplete="off">
  ...
  nameInput.addEventListener("input", () => {
    ...
    _selectedName = ""; // reset on new typing
  ...
  /**
   * Pick the best display-name for a suggestion based on what the user typed.
   * If the typed query is closer to the Japanese title, show Japanese;
   * if closer to the English title, show English; otherwise fallback.
   */
  function bestMatchName(item, query) {
    ...
    // both match or neither: prefer English if available, else Japanese
    return en || jp;
  }
  ...
  function pickAnime(name, img) {
    ...
    _selectedName = name; // store for DB
  ...
  function renderSeasons() {
    ...
    seasonsList.querySelectorAll(".aam_season_remove").forEach((el) => {
      el.addEventListener("click", (e) => removeSeason(+e.target.dataset.idx));
    });
  }
