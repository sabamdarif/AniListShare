...
def api_anime_list(request):
    ...
    anime_qs = Anime.objects.filter(category_id=category_id).prefetch_related('seasons').order_by('order')
    data = []
    for a in anime_qs:
        data.append({
            'id': a.id,
            ...
            'seasons': [
                {'id': s.id, 'label': s.label, 'comment': s.comment, 'order': s.order}
...
def api_anime_reorder(request):
    ...
    if not anime_id or direction not in ('up', 'down'):
        return JsonResponse({'error': 'anime_id and direction (up/down) required'}, status=400)
