...
import pyexcel_ods3
from animelist.models import Category, Anime, Season
...
def fetch_thumbnail(name, retries=3):
    for attempt in range(retries):
        try:
            ...
            req = urllib.request.Request(url, headers={'User-Agent': 'AnimeListMigration/1.0'})
...
def import_sheet(sheet_name, rows, order, limit=None):
    cat, created = Category.objects.get_or_create(name=sheet_name, defaults={'order': order})
    if not created:
...
def main():
    parser = argparse.ArgumentParser(description='Import animelist.ods into Django database')
    parser.add_argument('--limit', type=int, default=None, help='Limit rows per sheet (for testing)')
    parser.add_argument('--clear', action='store_true', help='Clear all existing data before import')
    parser.add_argument('--thumbnails-only', action='store_true', help='Only fetch thumbnails for existing entries (skip data import)')
    parser.add_argument('--no-thumbnails', action='store_true', help='Only import data without fetching thumbnails')
